...
class BeamSearchScorer(BeamScorer):
    ...
    def finalize(
        ...
        all_hyps = []
        all_scores = torch.zeros((batch_size, self.num_beams), device=self.device, dtype=torch.float32)
        all_hyp_lengths = input_ids.new(batch_size * self.num_beams)

        for i, beam_hyp in enumerate(self._beam_hyps):
            current_hyps = []
            for j in range(len(beam_hyp.beams)):
                score, hyp, _ = beam_hyp.beams[j]
                current_hyps.append(hyp)
                all_scores[i, j] = score
                all_hyp_lengths[self.num_beams * i + j] = len(hyp)
            
            all_hyps.append(current_hyps)
        ...
        hyp_lengths_max = all_hyp_lengths.max().item() + 1
        hyp_max_len = min(hyp_lengths_max, max_length) if max_length is not None else hyp_lengths_max
        all_decoded: torch.LongTensor = input_ids.new(batch_size, self.num_beams, hyp_max_len)
        ...
        if sent_lengths.min().item() != sent_lengths.max().item():
            ...
            all_decoded.fill_(pad_token_id)
        ...
        for i, current_hyps in enumerate(all_hyps):
            for j, hyp in enumerate(current_hyps):
                all_decoded[i, j, : all_hyp_lengths[self.num_beams * i + j]] = hyp
                if all_hyp_lengths[self.num_beams * i + j] < hyp_max_len:
                    all_decoded[i, j, all_hyp_lengths[self.num_beams * i + j]] = eos_token_id
        ...
        return UserDict(
            {
                "candidate_sequences": all_decoded,
                "candidate_sequence_scores": all_scores,
                "sequences": decoded,
